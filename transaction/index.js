...
class Transaction {
    ...
    static validateCreateAccountTransactions({ transaction }){
        return new Promise((resolve, reject) => {
            const expectedAccountData = Object.keys(new Account().toJSON())

            const fields = Object.keys(transaction.data.accountData)

            if(fields.length !== expectedAccountData.length){
                return reject(new Error(`The transaction account data has an incorrect number of fields`))
            }

            fields.forEach((field) => {
                if(!expectedAccountData.includes(field)){
